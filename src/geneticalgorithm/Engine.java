...
public class Engine {
	...
	public void run(Result res) throws IOException {
		...
		while (true) {
			...
			if (t.isAlive()) {
				...
				if(currentPlayer.equals(Checker.WHITE))
					result = 0;
				else result = 1;
			...
			try {
				...
				if (currentAction == null) {
					...
					if (currentPlayer == Checker.BLACK) {
						...
						result = 0;
						System.out.println(result);
						this.whiteSocket.writeState(null);
						this.blackSocket.writeState(null);
						break;
		...
		//this.whiteSocket.writeState(null);
		//this.blackSocket.writeState(null);
		this.whiteSocket.close();
		this.blackSocket.close();
		/*socket = new Socket("localhost", 5803);
