...
import geneticalgorithm.Parametres;
...
public class Negascout {
	...
	public String doSearch(State initState, double alpha, double beta, Parametres par) {
		...
		System.out.println("Muline Bleu starting search...");
		...
		while(!stop.getIsTimeout()) {
			...
			this.negascout(move, init, 0, alpha, beta, isPlayerTurn, par);
	...
	private double negascout(Move move, Successor current, int depth, 
		...
		for (int i = 0; i < successors.size(); i++) {
			...
			score = -negascout(move, successor, depth + 1, -b, -a, !isPlayerTurn, par);

			if (i == 0)
				action = successor.getAction();
			else {
				if (a < score && score < beta)
					score = -negascout(move, successor, depth + 1, -beta, -score, !isPlayerTurn, par);
			}
	...
	private double evaluate(Successor successor, Parametres par) {
		
		double result = 0;
		
		result += par.getOpenMill() * Heuristics.evaluateOpenMills(successor, player);
		result += par.getNumberOfPieces() * Heuristics.evaluateNumberOfPieces(successor, player);
		result += par.getBlockedMoves() * Heuristics.evaluateBlockedMoves(successor, player);
