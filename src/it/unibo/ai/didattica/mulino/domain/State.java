...
public class State implements Serializable, IState {
	...
	// aggiunto da MATTIA (20/04): metodo che resituisce le posizioni del tipo di pedina passato come parametro
	// con il parametro removable è possibile specificare se si vogliono solo le posizioni di pedine
	// che possono essere rimosse, quindi non quelle che formano un tris
	public List<String> getCheckerPositions(Checker checker, boolean removable) {
		List<String> positions = new ArrayList<String>();

		if(!removable) {
			for(String position : this.positions) {
				if(this.board.get(position).equals(checker))
					positions.add(position);
			}
		}
		else {
			for(String position : this.positions) {
				if(this.board.get(position).equals(checker) && !(Util.hasCompletedTriple(this, position, checker)))
					positions.add(position);
			}
		}
	...
	// aggiunto da MATTIA (20/04): metodo che restituisce il numero di pedine sulla scacchiera del giocatore passato come parametro

	public int getCheckersOnBoard(Checker currentPlayer) {
		if(currentPlayer.equals(Checker.BLACK))
			return this.blackCheckersOnBoard;
		else if(currentPlayer.equals(Checker.WHITE))
			return this.whiteCheckersOnBoard;
		else throw new IllegalArgumentException("Checker type not valid");
	}

	// aggiunto da MATTIA (20/04): metodo che diminuisce il numero di pedine da posizionare

	public void setCheckersToPut(Checker checker) {

		if(checker.equals(Checker.BLACK)) {
			this.blackCheckers = this.blackCheckers - 1;
		}
		else if(checker.equals(Checker.WHITE)) {
			this.whiteCheckers = this.whiteCheckers - 1;
		}
		else throw new IllegalArgumentException("Checker type not valid");
	}

	// aggiunto da MATTIA (20/04): metodo che aggiorna il numero di pedine sulla scacchiera del tipo passato come parametro

	public void setCheckersOnBoard(Checker checker, boolean add) {
		if(checker.equals(Checker.BLACK)) {
			if(add)
				this.blackCheckersOnBoard = this.blackCheckersOnBoard + 1;
			else
				this.blackCheckersOnBoard = this.blackCheckersOnBoard - 1;
		}
		else if(checker.equals(Checker.WHITE)) {
			if(add)
				this.whiteCheckersOnBoard = this.whiteCheckersOnBoard + 1;
			else
				this.whiteCheckersOnBoard = this.whiteCheckersOnBoard - 1;
		}
		else throw new IllegalArgumentException("Checker type not valid");
	...
	// aggiunto da MATTIA (20/04): metodo che restituisce il numero di pedine da posizionare del tipo passato come parametro

	public int getCheckers(Checker checker) {
		if(checker.equals(Checker.BLACK))
